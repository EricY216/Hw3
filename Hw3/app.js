...
function loadLogisticsOrders() {
    let url = "./logistics/logisticsControl.php?act=orders";
    fetch(url, {
        method: 'GET',
    })
    .then(function(response) {
    ...
    .then(function(data) {
        ...
        for (let r of data) {
            ...
            if (r['orderStatus'] === 2) {
                result += "<td><button onclick='changeLogisticsOrderStatus(" + r['orderID'] + "," + r['orderStatus'] + ")'>訂單已寄送</button></td>";
            } else if (r['orderStatus'] === 3) {
                result += "<td><button onclick='changeLogisticsOrderStatus(" + r['orderID'] + "," + r['orderStatus'] + ")'>訂單已送達</button></td>";
            } 
            result += "</tr>"
        }
        result += "</table>"
...
function changeLogisticsOrderStatus(id, status) {
    let url = './logistics/logisticsControl.php?act=changeOrderStatus&id=' + id +'&status=' + status;
    fetch(url, {
        method: 'GET',
    })
    .then(function(response) {
